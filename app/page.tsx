...
export default function PageBordereau() {
  ...
  const [valeurDeclaree, setValeurDeclaree] = useState(0);
  const [nombreColis, setNombreColis] = useState(1);
  const [poidsReel, setPoidsReel] = useState(0);
  ...
  const construireBordereau = (): Bordereau => ({
    numero,
    dateCreation: new Date().toLocaleDateString("fr-FR"),
    expediteur: ENTREPOT_EXPEDITEUR,
    destinataire: { nom: nom.trim(), prenom: prenom.trim(), rue: rue.trim(), codePostal: codePostal.trim(), ville: ville.trim(), territoire, indicatifPays, telephoneWhatsapp: telephoneWhatsapp.trim() },
    declaration: { description: description.trim(), valeurDeclaree, nombreColis },
  ...
  const NumberField = ({ label, value, setValue, step = "1", placeholder }: { label: string; value: number; setValue: (v: number) => void; step?: string; placeholder?: string }) => (
    <div><label className={labelClass}>{label}</label><input className={inputClass} type="number" min={0} step={step} value={value || ""} placeholder={placeholder} onChange={(e) => setValue(positive(e.target.value))} /></div>
  );
